...
Graph* graph_load_from_file(const char *filename) {
    ...
    while (fscanf(f, "%d %d", &u, &v) == 2) {
        if (u < 0 || u >= num_vertices || v < 0 || v >= num_vertices) continue;
    ...
    if (!g->adjacency_index || !g->adjacency_list) {
        for (int i = 0; i < num_vertices; i++) free(temp_adj[i]);
        free(temp_adj);
        free(temp_count);
        free(temp_cap);
        graph_free(g);
